...
"""
generates the UKHL corpus

@author: rozano aulia

TO DO - must fix error where case 1.19 was replicated x3
"""
...
def fetch_asmoval(asmo, checkpoint):
    with open('./data/consensus_asmo.csv', 'r') as infile:
...
def getAsmo(asmo, text, new_case, checked):
    ...
    with open('./data/ASMO_68_corpus/' + filename) as infile:
        reader = csv.DictReader(infile)
...
def appendcsv(case_id, asmo, asmo_sent, sentence_id, para_id, judge, text, role, align, agree, outcome, ackn, fieldnames):
    # with open('./data/test.csv', 'a', newline='') as out_file: #for quick testing
    with open('./data/UKHL_corpus.csv', 'a', newline='') as out_file:
...
def UKHL_corpus():
    ...
    "2003Mar20sepet-1.ling.xml", "2003Mar20sivak-1.ling.xml", "2003May22john-1.ling.xml", "2003Jan30regina-1.ling.xml", "2001Jan25montgo-1.ling.xml", "2001Jun28optid-1-NS.ling.xml",
    ...
    'N/A', '60', '49', '56', '66',
    '68', '52', '53',
    # cases from 0 - not annotated for relevance, just used to test cases
    '0', '1', '8',
    '10', '16', '25', '27', '28',
    '30', '33', '37', '39', '41',
    '44', '48', '50', '51', '54',
    ...
    directory = os.fsencode('./data/SUM_2004_corpus/')

    for file in os.listdir(directory):
        filename = os.fsdecode(file)

        if filename not in corpusList:
            with open('./data/corpus_list.csv', 'r') as corplist_file:
    ...
    # with open('./data/test.csv', 'w', newline='') as new_file: #for quick testing
    with open('./data/UKHL_corpus.csv', 'w', newline='') as new_file:
    ...
    for v in range(len(corpusList)):
        ...
        par_dp = False
        para_id = '0'
        role = '<new-case>' # NEW CASE ROLE FIRST
        align = 'NONE'
        text = ''
        agree = ''
        outcome = ''
